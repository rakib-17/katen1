				...
				<div class="collapse navbar-collapse">
					...
					<ul class="navbar-nav mr-auto">
                        ...
                        @foreach ($categories as $category)
                            <li class="nav-item {{ $category->sub_categories->count() > 0 ? 'dropdown': '' }}">
                                <a class="nav-link {{ $category->sub_categories->count() > 0 ? 'dropdown-toggle': '' }}" href="{{ route('category',$category->slug) }}">{{ $category->name }}</a>
                                @if($category->sub_categories->count() > 0)
                                    <ul class="dropdown-menu">
                                        @foreach ($category->sub_categories as $sub_category)
                                        <li><a class="dropdown-item" href="{{ route('category', $sub_category->slug) }}">{{ $sub_category->name }}</a></li>
                                        @endforeach
                                    </ul>
                                @endif
                            </li>
